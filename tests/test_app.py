...
class ExpenseManagerTestCase(unittest.TestCase):
    ...
    def test_add_expense_rejects_non_finite_amount(self):
        for bad_amount in ("nan", "inf", "-inf"):
            response = self.add_expense(amount=bad_amount)
            self.assertEqual(response.status_code, 200)
            self.assertIn(b"Amount must be a finite number.", response.data)
